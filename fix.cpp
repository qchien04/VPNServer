...
public:
    ...
    // Helper function to get main network interface
    std::string getMainInterface() {
        std::ifstream route_file("/proc/net/route");
        std::string line, interface, destination;
        
        if (!route_file.is_open()) {
            return "ens33"; // fallback to your system's interface
        }
    ...
    // Helper function to print network status
    ...
    // Create TUN interface with enhanced network configuration
    bool createTunInterface(const std::string& tun_name = "tun1") {
        ...
        if (is_server) {
            ...
            // Add specific NAT rule for LAN access through ens37
            cmd = "iptables -t nat -A POSTROUTING -s 10.0.0.0/24 -d 192.168.50.0/24 -o " + lan_interface + " -j MASQUERADE";
            std::cout << "Executing: " << cmd << std::endl;
            system(cmd.c_str());
            
            // Add NAT rule for internet access through main interface
            ...
            // Configure client TUN interface
            cmd = "ip addr add " + tunnel_ip + "/24 dev " + tun_name;
            std::cout << "Executing: " << cmd << std::endl;
            system(cmd.c_str());
...
int main() {
    ...
    if (mode == 's' || mode == 'S') {
        ...
        if (!vpn->initialize("0.0.0.0", "192.168.2.10")) {
